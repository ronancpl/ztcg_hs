ZTCG_CARD
{
    ...
    function onLevelUp(player)
        ...
        if isBossCARD(card) and not isBossOnPlayerTable(player) then
            ...
            if not isEmptyList(clist) then
                ...
                if card ~= 0 then
                    local card = takeTargetCardFromDeck(card,hand)
                    card = takeTargetCardFromListToDeck(hand,card,card,"DECK_BOTTOM")
    ...
    function onActivateCharacterAction(player)
        ...
        local clist_m = makeFilteredList(player, list, 0, "ZTCG_DONTCARE", "ZTCG_DONTCARE", "TYPE_MOB", "ELEM_ANY", "ZTCG_NIL")
        ...
        if not isEmptyList(clist_m) and not isEmptyList(clist_e) and not isEmptyList(clist_t) then
            if makePrompt(player,"Use Collect and Trade to draw cards?","Reveal each of monster, item and tactic to draw cards.","ZTCG_NIL","ZTCG_NIL","OK","Cancel") then
                ...
                if card_m ~= 0 and card_e ~= 0 and card_t ~= 0 then
                    ...
                    moveCards(hand,deck,"TAKE_CARDID","PUT_BOTTOM",card_m)
                    moveCards(hand,deck,"TAKE_CARDID","PUT_BOTTOM",card_e)
                    moveCards(hand,deck,"TAKE_CARDID","PUT_BOTTOM",card_t)
