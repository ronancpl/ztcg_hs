ZTCG_CARD
{
    ...
    function onThinkAction(player)
        ...
        editCardRegister(src, cid, 0, 11, 0, nil)
    ...
    function undoBuffs(player)
        local src = getSourceCARD()
        local cid = getCardIdFromCARD(src)
        if(getCardRegister(src, cid, 0) < 10 or (getCardRegister(src, cid, 0) == 10 and not matchRequirements(player, 50, 2, "ELEM_WARRIOR"))) then return end
